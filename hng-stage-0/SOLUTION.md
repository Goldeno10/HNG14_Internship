...
## 4. What you need to do on the database

Apply the new indexes to your Neon database:
...
This runs `prisma/migrations/20260504170000_stage4b_profile_read_indexes/migration.sql`.

Ensure **Upstash Redis** env vars remain set (`UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`) so caching and `profile:data_version` work.
...
| Area | Files |
|------|--------|
| Indexes | `prisma/schema.prisma`, `prisma/migrations/.../migration.sql` |
| Normalize / countries | `lib/query-normalize.ts`, `lib/country-aliases.ts`, `lib/nlp-parser.ts` |
| Cache / version | `lib/profile-cache.ts`, `lib/profile-data-version.ts` |
| Ingestion | `lib/profile-csv-ingest.ts`, `app/api/profiles/import/route.ts` |
| Routes | `app/api/profiles/route.ts`, `app/api/profiles/search/route.ts`, `app/api/profiles/[id]/route.ts` |
| Edge / CORS | `proxy.ts` |
| Deps | `package.json` (`csv-parse`, `busboy`, `@types/busboy`) |
...
1. **Migrate** (see §4).  
2. **List cache:** identical `GET /api/profiles?...` twice — second should be faster with Redis.  
3. **Normalization:** two different `q` strings that map to the same filters should hit the same cache after first request.  
4. **Import:** `curl -X POST -H "Authorization: Bearer …" -H "X-API-Version: 1" -F "file=@profiles.csv" https://…/api/profiles/import` — expect summary JSON with partial inserts on bad rows.
